...
describe('Twitch Routes', () => {
  ...
  describe('GET /twitch/streamers', () => {
    it('should return unvalidated streamers', async () => {
      const mockStreamers = [
        { _id: 'streamer1', id: 'twitch_id1', name: 'Streamer1', isValid: false },
        { _id: 'streamer2', id: 'twitch_id2', name: 'Streamer2', isValid: false },
      ];

      (Streamers.find as jest.Mock) = jest.fn().mockResolvedValue(mockStreamers);
  ...
  describe('GET /twitch/live', () => {
    it('should return live streamers to announce', async () => {
      const mockLiveStreamers = [
        { _id: 'streamer1', isLive: true, isAnnounce: false, name: 'Streamer1' },
      ];

      (Streamers.find as jest.Mock) = jest.fn().mockResolvedValue(mockLiveStreamers);
  ...
  describe('POST /twitch/streamers', () => {
    ...
    it('should add announcer to existing streamer', async () => {
      ...
      (Streamers.findOne as jest.Mock) = jest.fn().mockResolvedValue(mockExistingStreamer);
      (Streamers.findOneAndUpdate as jest.Mock) = jest.fn().mockResolvedValue({
        ...mockExistingStreamer,
        announcer: [...mockExistingStreamer.announcer, {
          guild_id: 'guild123',
          channel_id: 'channel123',
          role_id: 'role123',
          message: 'Live now!',
  ...
  describe('DELETE /twitch/streamers', () => {
    it('should remove announcer from streamer with multiple announcers', async () => {
      const mockStreamer = {
        id: 'twitch_id1',
        announcer: [
          { guild_id: 'guild123', channel_id: 'channel123', role_id: 'role123', message: '', progress: false },
          { guild_id: 'guild456', channel_id: 'channel456', role_id: 'role456', message: '', progress: false },
        ],
      };

      (Streamers.findOne as jest.Mock) = jest.fn().mockResolvedValue(mockStreamer);
      (Streamers.findOneAndUpdate as jest.Mock) = jest.fn().mockResolvedValue({
        id: 'twitch_id1',
        announcer: [{ guild_id: 'guild456', channel_id: 'channel456', role_id: 'role456', message: '', progress: false }],
      });
    ...
    it('should delete streamer with single announcer', async () => {
      const mockStreamer = {
        id: 'twitch_id1',
        announcer: [{ guild_id: 'guild123', channel_id: 'channel123', role_id: 'role123', message: '', progress: false }],
      };

      (Streamers.findOne as jest.Mock) = jest.fn().mockResolvedValue(mockStreamer);
      (Streamers.deleteOne as jest.Mock) = jest.fn().mockResolvedValue({ deletedCount: 1 });
