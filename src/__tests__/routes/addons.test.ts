...
describe('Addons Routes', () => {
  ...
  describe('GET /addons', () => {
    it('should return all addons for a guild', async () => {
      const mockAddons = [
        { _id: 'addon1', guild_id: 'guild123', name: 'Twitch', active: true, channel: 'channel_id', role: 'role_id' },
        { _id: 'addon2', guild_id: 'guild123', name: 'DailyUI', active: true, channel: 'channel_id2', role: 'role_id2' },
      ];
  ...
  describe('PUT /addons/update', () => {
    it('should update an addon', async () => {
      ...
      (Addons.findByIdAndUpdate as jest.Mock) = jest.fn().mockResolvedValue(mockAddon);
