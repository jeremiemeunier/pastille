...
export const automodVerifier = async (guild: Guild) => {
  ...
  Logs({
    node: ["automod", "verifier"],
    state: null,
    content: "start sanctions verifications",
    details: guild?.id,
  });
  try {
        ...
        allGuildSanctions.map(async (item: SanctionTypes) => {
          ...
          const userFetch = async () => {
            try {
              ...
              Logs({
                node: ["automod", "verifier", "fetch_user"],
                state: "error",
                content: err,
                details: guild?.id,
              });
          ...
          if (!user) {
            ...
            Logs({
              node: ["automod", "verifier", "rebind"],
              state: "warning",
              content: `user not find : ${user_id}`,
              details: guild?.id,
            });
            return;
          }
          if (!sanctionRole) {
            Logs({
              node: ["automod", "verifier", "rebind"],
              state: "warning",
              content: `role not find : ${moderation.roles.muted}`,
              details: guild?.id,
            });
            return;
          }
          ...
          if (ending <= now) {
            ...
            try {
              ...
              Logs({
                node: ["sanction", "verifier", "remove", "timer"],
                state: "error",
                content: err,
                details: guild?.id,
              });
  ...
  Logs({
    node: ["automod", "verifier"],
    state: null,
    content: "end sanctions verifications",
    details: guild?.id,
  });
};
