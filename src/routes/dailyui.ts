...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(201).json({ message: "State updated for DailyUi", data: q_update });
    } catch (err: any) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (q_dailyui) {
        res.status(200).json({ message: "DailyUi available", data: q_dailyui });
        return;
      }
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      ...
      try {
        ...
        res.status(200).json({ message: "New daily challenge added", data: q_make });
      } catch (err: any) {
        res.status(500).json({ message: "Internal server error", error: err });
