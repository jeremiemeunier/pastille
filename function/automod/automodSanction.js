...
const automodSanctionEvalute = async (size, guild) => {
  ...
  if(
    ...
    return { decision: "low", duration: duration, end: endTime, start: startTime }
  }
  else if(
    size === 12 ||
    size === 15 ||
    ...
    return { decision: "medium", duration: duration, end: endTime, start: startTime }
  }
  else if(
    size === 21 ||
    size === 24 ||
    ...
    return { decision: "hight", duration: duration, end: endTime, start: startTime }
  }
  else if(
    size > 27
  ) {
...
const automodSanction = async (user, size, guild) => {
  ...
  if(evaluation.decision === "nothing") { return; }
  if(evaluation.decision === "ban") {
    ...
    sanctionRegister(user.user.id, evaluation.decision, evaluation.start, evaluation.end, guild);
  }
}
